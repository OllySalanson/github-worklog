...
- Security invariant: the unencrypted page, the config and the password must never be written to disk or git, nor printed. `src/cli.js` renders in memory and hands only the output of `lockPage` to `build`/`publish`; keep it that way and keep the tests in `test/cli.test.js` that assert it.
